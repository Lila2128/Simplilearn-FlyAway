...
<% String Response=(String) request.getAttribute("Response");
	...
	<div class="Places-Master">
		<h4 class="Places-title"> Places</h4>
		<a href="PlacesNew.jsp?username=<%=request.getParameter("username")%>"><button type="button" class="btn btn-primary link-button">Add new Place</button></a>
	</div>
	<%int count=1; %>
	...
	<div class="table-Places" >
		...
		<table class="table" border="1">
			<tr><th class="row1">#</th><th class="row1">Source</th><th class="row1">Destination</th><th class="row1">Action</th></tr>
    	<% for(Places p:plist){ %>
    	
    	<!-- -----------------Row sequence No order wise should be done.--------------------- -->
    	<tr class="rows"><td><%=count %></td><td><%=p.getSource() %></td><td><%=p.getDestination() %></td>
    	<td><a href="placesDelete.jsp?source=<%=p.getSource() %>&username=<%=request.getParameter("username")%>">Delete</a></td></tr>
    	<% count=count+1; } %>
		</table>
		</div>
	</div>
